...
package com.isoftbao.zabbixj.agent.active;
...
import com.isoftbao.zabbixj.ZabbixException;
...
public class ActiveThread extends Thread {
	...
	private ActiveChecksResponseIndex getActiveChecksResponseIndex(JSONObject response) throws JSONException {
		...
		for(int i = 0; i < data.size(); i++) {
			...
			int delay = check.getIntValue("delay");
			index.add(key, delay);
		}
	...
	private JSONObject getResponse(byte[] responseBytes) throws Exception {
		...
		return JSONObject.parseObject(new String(jsonBuffer));
	}
